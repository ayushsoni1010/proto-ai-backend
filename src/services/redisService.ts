...
class RedisService {
  ...
  constructor() {
    ...
    for (const [name, conn] of [
      ...
      conn.on('reconnecting', () => logger.warn(`Redis ${name} reconnecting...`));
    }
  }
  ...
  // Pub/Sub
  async publishImageEvent(type: ImageEventType, payload: ImageEventPayload): Promise<number> {
  ...
  async subscribeToImageEvents(handler: (type: ImageEventType, payload: ImageEventPayload) => void): Promise<void> {
    await this.subscriber.subscribe(redisConfig.imageEventsChannel);
    this.subscriber.on('message', (_channel, message) => {
      try {
        const parsed = JSON.parse(message) as { type: ImageEventType; payload: ImageEventPayload };
  ...
  async setCompressed(key: string, value: unknown, ttlSeconds?: number): Promise<void> {
  ...
  async setChunk(key: string, chunkIndex: number, buffer: Buffer): Promise<void> {

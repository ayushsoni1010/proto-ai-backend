  ...
  async uploadImage(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        ...
        if (finalMimeType === "image/heic" || file.originalname.toLowerCase().endsWith(".heic")) {
      ...
      await s3Service.uploadFile(s3Key, processedBuffer, finalMimeType);
        ...
        if (finalMimeType === "image/heic" || file.originalname.toLowerCase().endsWith(".heic")) {
          ...
          logger.warn('Using default dimensions for HEIC file as Sharp cannot process it');
        } else {
          imageInfo = await imageProcessingService.getImageMetadata(processedBuffer);
        }
